...
var Application = function()
{
    ...
    this.ajaxBaseUrl = "http://musicovery.com/api/V3/track.php";
}
...
Application.prototype.start = function()
{
    ...
    if (search != null)
    {
        search.addEventListener("keydown", this.onAjaxSearch);
    }
...
/*
* Méthode de recherche AJAX
*/
Application.prototype.onAjaxSearch = function(event)
{
    setTimeout(function()
    {
        // Recuperation du texte a rechercher
        ...
        // Définition du type de methode HTTP et de l'URL distante
        var title = encodeURIComponent(searchText);
        var url = window.application.ajaxBaseUrl + "?fct=search&title=" + title + "&format=json";
        xhr.open("GET", url);

        // Ajout d'un écouteur pour traiter la réponse
        xhr.addEventListener("readystatechange", function()
        {
            if ((xhr.readyState === XMLHttpRequest.DONE) && (xhr.status === 200))
            {
                try
                {
                    ...
                    if ((response.root != null) && (response.root.tracks != null) && (response.root.tracks.track != null) && (response.root.tracks.track.length > 0))
                    {
                        for (var i = 0; i < response.root.tracks.track.length; i++)
                        {
                            var track = response.root.tracks.track[i];
                            if ((track.title != null) && (track.artist.name != null))
                            {
                                var song = {title: track.title, artist: track.artist.name};
                                songsFound.push(song);
                            }
                        }
                    }

                    // Affiche les musique trouvées
                    window.application.displaySongs(songsFound);
                }
                catch (error)
                {
